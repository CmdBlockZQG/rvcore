#ifndef __CORE_EMU_DECODE_H__
#define __CORE_EMU_DECODE_H__
...
static inline uint64_t sbits(const uint64_t x) {
  return sext<H - L + 1>(bits<H, L>(x));
}

// -------------------- 立即数 --------------------

static inline word_t immI(const inst_t inst) {
  return sbits<31, 20>(inst);
}

static inline word_t immS(const inst_t inst) {
  return sbits<31, 25>(inst) << 5 | bits<11, 7>(inst);
}

static inline word_t immB(const inst_t inst) {
...
static inline word_t immU(const inst_t inst) {
  return sbits<31, 12>(inst) << 12;
}

static inline word_t immJ(const inst_t inst) {
...
static inline word_t zimm(const inst_t inst) {
  return bits<19, 15>(inst);
}

// -------------------- 指令 --------------------
static inline inst_t c_opcode(const inst_t inst) {
  return bits<1, 0>(inst);
}

static inline inst_t opcode(const inst_t inst) {
  return bits<6, 2>(inst);
}

static inline inst_t rs1(const inst_t inst) {
  return bits<19, 15>(inst);
}

static inline inst_t rs2(const inst_t inst) {
  return bits<24, 20>(inst);
}

static inline inst_t rd(const inst_t inst) {
  return bits<11, 7>(inst);
}

static inline inst_t funct3(const inst_t inst) {
  return bits<14, 12>(inst);
}

static inline inst_t funct5(const inst_t inst) {
  return bits<31, 27>(inst);
}

static inline inst_t funct7(const inst_t inst) {
  return bits<31, 25>(inst);
}

static inline inst_t funct12(const inst_t inst) {
